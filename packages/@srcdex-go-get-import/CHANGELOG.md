...
- Development aids on the router factory, both off by
  default: `resolve` answers `?resolve=1` with the JSON
  resolution of the URL, and `logger` receives each
  ...
  over `console` or a drop-in such as consola. The
  middleware reads no environment of its own.
